...
let users: any[] = persistedData?.users || [];
...
let settings = persistedData?.settings || {
...
let quotes: any[] = persistedData?.quotes || [];

// In-memory employees store
let employees: any[] = persistedData?.employees || [];

// In-memory purchasable services store (Admin managed)
let purchasableServices: any[] = persistedData?.purchasableServices || [];

// In-memory purchased services store
let purchasedServices: any[] = persistedData?.purchasedServices || [];

// In-memory refund requests store
let refundRequests: any[] = persistedData?.refundRequests || [];
...
async function startServer() {
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      try {
        ...
        // We don't have the Firebase UID here, but we can use the email as a temporary link 
        // OR we can wait for the client to sync it after Firebase Auth login.
        // Actually, the client-side is better for linking UID.
      } catch (fsError) {
        console.error("Error syncing user to Firestore:", fsError);
      }
      ...
      res.json({ 
        ...
        user: { 
          ...
          email: user.email, 
          role: user.role || (user.email === '106077az@gmail.com' ? 'admin' : 'user'),
          customerId: user.customerId, // Include customerId for employees
  ...
  app.post("/api/quotes/submit", async (req, res) => {
    try {
      ...
      syncData();
      console.log("New Quote Received:", quoteData.id);
  ...
  app.post("/api/customers", async (req, res) => {
    try {
      ...
      // Default password for new customers added by admin
      const defaultPassword = "Emin" + Math.floor(1000 + Math.random() * 9000);
          ...
          html: `
            <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
              ...
              <div style="background: #f4f4f5; padding: 20px; border-radius: 8px; margin: 20px 0;">
                ...
                <p style="margin: 0;">Geçici Şifre: <strong>${defaultPassword}</strong></p>
              </div>
