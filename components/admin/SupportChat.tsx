...
export const SupportChat: React.FC<SupportChatProps> = ({ user, isAdminView = false }) => {
      ...
      if (fUser) {
        ...
        try {
          ...
          await setDoc(userRef, {
            role: user.role || 'user',
            customerId: user.role === 'Çalışan' ? String(user.customerId) : String(user.id),
            email: user.email,
            name: user.name,
  ...
  useEffect(() => {
    ...
    // Fetch tickets
    // If admin, we'll try to fetch all. If it fails due to permissions, we'll show a message.
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // If admin and permission denied, it means they need to be elevated
      if (isAdminView && error.message.includes('permission-denied')) {
        console.warn("Admin access denied. Anonymous session may need elevation.");
      }
      setIsLoading(false);
    });

    return () => unsubscribe();
  }, [firebaseUser, isAdminView]);
  ...
  const handleCreateTicket = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newTicketSubject.trim() || !newTicketMessage.trim() || !firebaseUser) return;
    ...
    try {
      ...
      const ticketData = {
        ...
        customerEmail: user.email || 'anonymous@site.com',
      ...
      const newTicketDoc = await getDoc(docRef);
      if (newTicketDoc.exists()) {
        setSelectedTicket({ id: newTicketDoc.id, ...newTicketDoc.data() } as Ticket);
      }
    } catch (error: any) {
      console.error("Error creating ticket:", error);
      
      // Enhanced error reporting for AIS Agent
      const errInfo = {
        error: error.message,
        code: error.code,
  ...
  const elevateToAdmin = async () => {
    if (!firebaseUser || !isAdminView) return;
    setIsElevating(true);
    try {
      await setDoc(doc(db, 'admins', firebaseUser.uid), {
        elevatedAt: serverTimestamp(),
        siteEmail: user.email,
        name: user.name
      });
      // Force a re-fetch by toggling a state or just waiting for snapshot to retry
      // Actually, the snapshot should automatically retry or we can just reload the component's ticket fetch
      setTickets([]); // Clear to trigger loading state
      setIsLoading(true);
      // The useEffect for tickets will re-run because we're changing isLoading
    } catch (error) {
      console.error("Elevation failed:", error);
      alert("Yönetici yetkisi alınamadı. Lütfen sistem yöneticisine başvurun.");
    } finally {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newMessage.trim() || !selectedTicket || !firebaseUser) return;
    ...
    try {
      ...
      if (isAdminView) {
        updateData.status = 'pending'; // Waiting for customer
      } else {
        updateData.status = 'open'; // Waiting for admin
      }
    ...
    return (
      <div className="h-[600px] flex flex-col items-center justify-center bg-card rounded-[2.5rem] border border-border shadow-xl p-12 text-center space-y-8">
        ...
        <div className="space-y-3">
          <h3 className="text-3xl font-black text-foreground tracking-tight">Destek Sistemine Bağlanılıyor</h3>
            ...
            {filteredTickets.length === 0 ? (
              <div className="text-center py-20 opacity-40">
                ...
                {isAdminView && (
                  ...
                  >
                    {isElevating ? 'Yetkilendiriliyor...' : 'Yönetici Yetkisini Aktif Et'}
                  </Button>
                )}
              </div>
            ) : (
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-[9px] font-bold text-muted-foreground/60">
                      {ticket.updatedAt?.toDate().toLocaleDateString('tr-TR')}
                    </span>
                  </div>
